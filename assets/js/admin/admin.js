...
function renderLogin(message = '') {
  ...
  document.getElementById('adminLoginBtn').onclick = async () => { try { const data = await fetch(`${API_BASE_URL}/api/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: document.getElementById('adminEmail').value.trim(), password: document.getElementById('adminPassword').value }) }).then(r => r.json()); if (!data.token || !['ADMIN', 'ROLE_ADMIN'].includes(data.user?.role)) throw new Error(data.message || 'Admin account required.'); localStorage.setItem('authToken', data.token); localStorage.setItem('currentUser', JSON.stringify(data.user)); loadPage('dashboard'); } catch (error) { renderLogin(error.message); } };
}
